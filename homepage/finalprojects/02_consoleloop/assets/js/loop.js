//loop function fired onclick html button
function loop() {
//timer for loop
var timer = 0;
//line counter for loop
var counter = 0;
//content of loop in array
...
var arrayLength = array.length;
	...
	//setTimeout loop to run on html button click
...
//other stuff here that didn't work but might be of some use
/*
function block(emojis, word, leftCount) {
  //  emojis = 'fadjkladghklgajklagsdf'
  // word = 'eaererty'
  var lines = [];
  var leftString = '';
  var centerString = '';
  var rightString = '';
  centerString = word;
  leftString = emojis.substring(0, leftCount);
  rightString = emojis.substring(0, leftCount);
  var lineNumber = 0;
  while (centerString.length >= 2) {
    var aline = leftString + centerString + rightString;
    lines.push(aline);
    var nextEmoji = emojis[Math.min(
            emojis.length-1, Math.round(
                Math.random()*emojis.length))]
    leftString+= nextEmoji;
    rightString = leftString;
    centerString.length = centerString.length-1;
    centerString = centerString.substring(1);    
  }
  var topHalfCount = lines.length;
  for (var i = 1; i <= topHalfCount; i++) {
    lines.push(lines[topHalfCount - i]);
  }  
